...
def test_step_benchmark_sglang():
    ...
    analyzer = StepBenchmarkThroughputAnalyzer(runtime_args=runtime_args, benchmark_metrics=metrics)
    result = analyzer.run()
...
@pytest.mark.skipif(not check_framework("tensorrt_llm"), reason="tensorrt_llm is not installed")
@pytest.mark.skipif(True, reason="Hook is invalid in pytest environment.")
