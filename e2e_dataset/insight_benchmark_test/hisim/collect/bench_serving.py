...
async def _call_profile_pd(profile_urls: list[tuple[str, str]], mode: str) -> None:
    ...
    for worker_type, url in profile_urls:
        ...
        if profile_output.success:
            ...
            print(f"Failed to {mode} profiler for {worker_type} worker at {url}: {profile_output.error}")
...
def sample_mmmu_requests(
    ...
    try:
        ...
        print(f"Successfully loaded MMMU Math dataset from HuggingFace with {len(mmmu_dataset)} examples")
    ...
    for i, example in enumerate(sample_dataset):
        try:
            ...
            if image is not None:
                ...
                data_row = create_mm_data_row(text_prompt, [image], [image_data], output_len, processor, backend)
...
def sample_random_requests(
    ...
    if random_sample:
        ...
        for i in range(num_prompts):
            input_content = [(offsets[i] + i + j) % tokenizer.vocab_size for j in range(input_lens[i])]
            if return_text:
                input_content = tokenizer.decode(input_content)
            input_requests.append(
                DatasetRow(
...
def sample_image_requests(
    ...
    # Sample text lengths
    input_lens = np.random.randint(max(int(input_len * range_ratio), 1), input_len + 1, size=num_requests)
    output_lens = np.random.randint(int(output_len * range_ratio), output_len + 1, size=num_requests)
...
def calculate_metrics(
    ...
    use_retokenized_itl = (
        accept_length is not None and accept_length > 0 and backend in ("sglang-oai", "sglang-oai-chat")
    )
        ...
        if outputs[i].success:
            ...
            if use_retokenized_itl:
                for k, itl in enumerate(outputs[i].itl):
                    num_tokens = len(tokenizer.encode(outputs[i].text_chunks[k], add_special_tokens=False))
...
async def benchmark(
    ...
    if profile and pd_separated:
        ...
        if not pd_profile_urls:
            print("Warning: PD separated mode requires --profile-prefill-url or --profile-decode-url")
    ...
    if "sglang" in backend:
        server_info = requests.get(base_url + "/get_server_info", headers=get_auth_headers())
...
def run_benchmark(args_: argparse.Namespace):
    ...
    if args.backend in ["sglang", "sglang-native"]:
        ...
        api_url = (
            f"{args.base_url}/v1/completions" if args.base_url else f"http://{args.host}:{args.port}/v1/completions"
        )
    ...
    lora_request_distribution = args.lora_request_distribution if args.lora_name is not None else None

    lora_zipf_alpha = (
        args.lora_zipf_alpha if args.lora_name is not None and args.lora_request_distribution == "skewed" else None
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-details", action="store_true", help="Output details of benchmarking.")
    parser.add_argument(
        "--disable-tqdm",
        action="store_true",
