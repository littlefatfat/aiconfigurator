    ...
    for line in open(real_requests_path).readlines():
        ...
        requests.append(
            FakeRequest(
                ...
                final_reused_tokens=item.get("final_prefix_cache_len", item.get("final_device_hit_len", 0)),
    ...
    for rate_dir in os.listdir(real_data_dir):
        for run_case in case_list:
            ...
            for predictor_name in predictors:
                metrics_file = sim_data_dir / rate_dir / f"{predictor_name}.{run_case}.metrics.json"
...
def show_metrics(
    ...
    for i, case in enumerate(cases):
        ...
        for j, metrics_name in enumerate(metrics_list):
            ...
            if metrics_name == "prefix_hit_ratio" or metrics_name == "disk_prefetch_ratio":
                ax.set_ylim(0, 1)
            else:
                ax.set_ylim(0)
    ...
    for i, case in enumerate(cases):
        ...
        y = (pos.y0 + pos.y1) / 2
        x = pos.x0 - 0.04
        fig.text(x, y, f"{case}", ha="right", va="center", rotation=90, fontweight="bold")
...
def cal_metrics_mpe(csv_path: Path):
    ...
    for m in metrics:
        ...
        merged[f"mpe_{m}"] = np.where((diff == 0) & diff.notna() & real_val.notna(), 0.0, diff / real_val)
    ...
    result_df = mock_df.groupby(["predictor", "case"])[mpe_columns].apply(lambda x: x.abs().mean())
    ...
    essential_result_df.to_csv(csv_path.parent / "essential_mpe_summary.csv", index=False)
