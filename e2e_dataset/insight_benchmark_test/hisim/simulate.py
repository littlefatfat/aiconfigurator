...
def run_simulation(
    ...
    # Get server info
    with open(model_bench_dir / "data" / os.listdir(model_bench_dir / "data")[0] / f"{case_list[0]}.metrics.json") as f:
        ...
        hicache_storage_prefetch_policy = real_metrics["server_info"]["hicache_storage_prefetch_policy"]
        disable_overlap_schedule = real_metrics["server_info"]["disable_overlap_schedule"]
    ...
    for request_rate_dir in model_bench_dir.joinpath("data").iterdir():
        ...
        for case_name in case_list:
            ...
            with open(
                output_dir / request_rate_dir.name / f"{predictor_name}.{case_name}.metrics.json",
                "w",
            ) as f:
                json.dump(metrics, f)
            ...
            if hasattr(runner, "get_iteration_stats"):
                with open(
                    output_dir / request_rate_dir.name / f"{predictor_name}.{case_name}.iteration.jsonl",
                    "w",
                ) as f:
            ...
            if hasattr(runner, "get_request_stats"):
                with open(
                    output_dir / request_rate_dir.name / f"{predictor_name}.{case_name}.request.jsonl",
                    "w",
                ) as f:
