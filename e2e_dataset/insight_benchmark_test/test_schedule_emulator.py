...
def test_benchmark_emulation():
    ...
    for _ in range(2):
        ...
        assert all([len(stats.request_stats) <= scheduler_config.max_running_requests for stats in iter_stats])
...
def test_disagg_emulation():
    ...
    p_scheduler_config = SchedulerConfig("Qwen2.5-3B", enable_stats=True, scenario="disagg_prefill")
    d_scheduler_config = SchedulerConfig("Qwen2.5-3B", enable_stats=True, scenario="disagg_decode")
