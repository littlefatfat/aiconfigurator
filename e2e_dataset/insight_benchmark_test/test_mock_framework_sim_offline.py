...
@pytest.mark.skipif(not check_framework("sglang", device="cpu"), reason="sglang is not installed.")
@pytest.mark.skipif(
...
def test_benchmark_sglang():
    ...
    for idx, req in enumerate(request_stats):
        ...
        assert dataset_args.min_input_len <= req["input_length"] <= dataset_args.max_input_len
