...
def test_aic_xgb_predictor():
    ...
    config = SchedulerConfig(model=model, backend_name="sglang", backend_version="0.5.6.post2")

    xgb_model_path = cur_dir / "assets/aic_xgb_models/"
    predictor = AIConfiguratorTimePredictor(model, hw, config, xgb_model_path=xgb_model_path)
