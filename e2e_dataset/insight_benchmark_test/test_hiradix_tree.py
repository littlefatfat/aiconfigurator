...
class req_factory:
    ...
    def get_req(
        ...
        if not self.tree_cache:
            return req
        match_result = self.tree_cache.match_prefix(key=RadixKey(token_ids=req.origin_input_ids, extra_key=None))
...
def decode_stage(
    ...
    if mem_pool.available_size < num_tokens:
        print(f"decode stage: try to evict {num_tokens}, {mem_pool.available_size=}, {mem_pool.evictable_size=}")
        tree_cache.evict(num_tokens)
        if mem_pool.available_size < num_tokens:
            print("decode stage: OOM Error!! no enough space in mem pool")
            tree_cache.pretty_print()
            return req
    print(f"decode stage: alloc {num_tokens}, {mem_pool.available_size=}, {mem_pool.evictable_size=}")
...
def test_radix_tree_loadback():
    ...
    req0 = factory.get_req([1, 2])
    print(f"req0 prefix_indices_len is {req0.prefix_indices_len}, host_hit_len is {req0.host_hit_len}")
    # 推理获取output_ids
    output_ids = [3, 7, 100]
    # tree.pretty_print()
    decode_stage(req0, output_ids, tree, kv_pool)
    ...
    print(f"req1 prefix_indices_len is {req1.prefix_indices_len}, host_hit_len is {req1.host_hit_len}")
    output_ids = [9, 10, 100]
    decode_stage(req1, output_ids, tree, kv_pool)
    process_finished_req(req1, tree, kv_pool)

    req2 = factory.get_req([1, 2, 3, 4])
    print(f"req2 prefix_indices_len is {req2.prefix_indices_len}, host_hit_len is {req2.host_hit_len}")
    output_ids = [11, 12, 100]
    decode_stage(req2, output_ids, tree, kv_pool)
    process_finished_req(req2, tree, kv_pool)

    req3 = factory.get_req([4, 4, 4, 4, 4, 4, 4, 4, 4])
    print(f"req3 prefix_indices_len is {req3.prefix_indices_len}, host_hit_len is {req3.host_hit_len}")
    output_ids = [4, 4, 4, 100]
    decode_stage(req3, output_ids, tree, kv_pool)
    process_finished_req(req3, tree, kv_pool)

    req4 = factory.get_req([1, 2, 3, 4, 5, 6])
    print(f"req4 prefix_indices_len is {req4.prefix_indices_len}, host_hit_len is {req4.host_hit_len}")
    output_ids = [13, 14, 100]
