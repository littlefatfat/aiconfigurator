...
def test_filter_machine():
    result = filter_machine(ModelInfo.find_by_model_name("Qwen2.5-7B"), framework_name="sglang")
    assert len(result) != 0
    result = filter_machine(ModelInfo.find_by_model_name("Qwen3-235B-A22B"), framework_name="sglang")
    assert len(result) != 0
...
def test_estimate_performance_sla_single_config():
    result = estimate_performance_sla_single_config(
        ...
        inference_config_args=InferenceConfigArgs(frameworks=["sglang"], data_types=["FP16"]),
    )
    assert len(result) != 0
...
def test_estimate_optimization_options():
    result = estimate_optimization_options(
        ...
        inference_config_args=InferenceConfigArgs(tp_sizes=[1], frameworks=["sglang"], data_types=["FP16"]),
    )
    assert len(result) != 0
...
def test_estimate_max_throughput():
    ...
    result = estimate_max_throughput(model=model, device=device, infer_config=infer_config)
