...
class req_factory:
    ...
    def get_req(self, req_to_token_pool: ReqToTokenPoolHost = None, token_ids=None) -> FakeRequest:
...
def test_radix_tree_with_req():
    ...
    tree_cache = RadixCache(req_to_token_pool=req_pool, kv_pool=kv_pool, page_size=1, disable=False)
    factory = req_factory()
    ...
    def req_match_prefix(req: FakeRequest):
        match_result = tree_cache.match_prefix(key=RadixKey(token_ids=req.origin_input_ids, extra_key=None))
