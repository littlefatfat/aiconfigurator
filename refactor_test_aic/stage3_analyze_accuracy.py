...
def analyze_mape(csv_path: str, title: str, plot_path: str, stats_path: str, count_threshold: int = 10) -> None:
    ...
    plt.fill_between(
        stats["batch_size"], stats["min_mape"], stats["max_mape"], color="tab:blue", alpha=0.2, label="MAPE Range"
    )
...
def _plot_error_buckets(
    df: pd.DataFrame, out_png: str, title: str, step: int = 1, clip: int = 30, annotate: bool = True
) -> None:
    """绘制有符号误差桶柱状图。"""
    df = df.copy()
    if clip and clip > 0:
...
def analyze_signed_error(
    prefill_csv: str, decode_csv: str, out_dir: str, step: int = 1, clip: int = 30, clip_0: int = 0
) -> None:
