...
def _get_session():
    ...
    from aiconfigurator.sdk import config as aic_cfg
    ...
    model = models.get_model(model_path=MODEL_PATH, model_config=model_config, backend_name=BACKEND_NAME)
...
def run_aic_estimation(data_dir: str) -> tuple[str, str]:
    ...
    logger.info("[Stage 2] 开始 AIC 时延预估")
    ...
    def _process(items, is_decode):
        ...
        processed.sort(key=lambda x: (-(x["ape"]) if x["ape"] >= 0 and not np.isinf(x["ape"]) else float("inf"),))
