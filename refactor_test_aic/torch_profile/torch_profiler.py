...
def _start_profile_http(
    ...
    print("[profiler] POST /start_profile:")
    for k, v in payload.items():
        print(f"  {k} = {v}")

    try:
        resp = _requests.post(
            f"{server_url}/start_profile",
            json=payload,
            timeout=600,
        )
    ...
    try:
        ...
        if args.launch_server:
            ...
            if not _health_check(server_url):
                ...
                print("    --json-model-override-args '{\"num_hidden_layers\": 2}' \\")
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            for i in range(args.num_requests):
                ...
                request_futures.append(executor.submit(_send_generate, server_url, prompt_ids, args.output_len, i))
            ...
            while extra_round < max_extra_rounds:
                ...
                for i in range(max(args.num_requests, 3)):
                    if prefix_token_ids:
                        ...
                        prompt_ids = _build_token_ids(args.input_len, seed=1000 + extra_round * 100 + i)
                    extra_futures.append(
                        executor.submit(
                            _send_generate, server_url, prompt_ids, args.output_len, f"extra-{extra_round}-{i}"
                        )
                    )
...
def run_engine_profiling(args) -> None:
    ...
    _ = llm.generate(input_ids=prefix_prompts, sampling_params={"temperature": 0, "top_p": 1, "max_new_tokens": 1})
        ...
        if st["active"] and batch is not None:
            ...
            if st["rem"] <= 0:
                ...
                tbl = (
                    ...
                    .table(sort_by="self_cuda_time_total", row_limit=200)
                )
...
""",
    ...
    prof.add_argument("--input-len", type=int, default=128, help="输入 token 长度 (默认: 128)")
    prof.add_argument("--prefix-len", type=int, default=0, help="prefix cache 长度 (默认: 0)")
    prof.add_argument("--output-len", type=int, default=64, help="最大输出 token 数 (默认: 64)")
    prof.add_argument("--num-requests", type=int, default=1, help="并发请求数 (默认: 1)")
    prof.add_argument("--num-steps", type=int, default=5, help="每阶段采集 forward step 数 (默认: 5)")
    prof.add_argument(
        "--profile-by-stage", action="store_true", default=True, help="分别采集 prefill/decode trace (默认: True)"
    )
    prof.add_argument("--no-profile-by-stage", dest="profile_by_stage", action="store_false")
    prof.add_argument(
        "--profile-stages", type=str, default=None, help="只 profile 指定阶段, 逗号分隔 (如 EXTEND,DECODE)"
    )
    prof.add_argument("--with-stack", action="store_true", default=True, help="记录 Python 调用栈 (默认: True)")
    prof.add_argument("--no-with-stack", dest="with_stack", action="store_false", help="禁用 Python 调用栈")
    prof.add_argument("--record-shapes", action="store_true", default=True)
    prof.add_argument("--warmup-runs", type=int, default=1)
    prof.add_argument(
        "--output-dir", type=str, default="/tmp/sglang_profile", help="trace 输出目录 (默认: /tmp/sglang_profile)"
    )
    ...
    srv.add_argument("--launch-server", action="store_true", help="自动启动 SGLang server")
    srv.add_argument("--model-path", type=str, default=None, help="模型路径 (也可通过 extra sglang args 传入)")
    srv.add_argument("--num-layers", type=int, default=None, help="覆盖 num_hidden_layers (减少层数加速 profiling)")
    srv.add_argument("--launch-timeout", type=float, default=600, help="等待 server 启动超时 (秒, 默认: 600)")
    srv.add_argument(
        "--auto-shutdown", action="store_true", default=True, help="profiling 完成后自动停止 server (默认: True)"
    )
    srv.add_argument("--no-auto-shutdown", dest="auto_shutdown", action="store_false")
