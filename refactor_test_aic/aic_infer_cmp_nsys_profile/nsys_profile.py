...
def run_profile(args):
    ...
    if not args.dry_run:
        ...
        _docker_exec(
            args.container, f"ls -lh {output_file} 2>/dev/null || echo '[error] Output file not found: {output_file}'"
        )
...
示例:
  ...
  python nsys_profile.py --mode prefill --case-id 321 --dry-run
    ...
    # 核心参数
    ap.add_argument("--mode", choices=["prefill", "decode"], help="profiling 模式: prefill 或 decode")
    ap.add_argument("--case-id", type=int, help="CSV 中的 case_id (即 JSONL 0-based 行号)")

    # 环境参数
    ap.add_argument(
        "--container", type=str, default=DEFAULT_CONTAINER, help=f"Docker 容器名 (default: {DEFAULT_CONTAINER})"
    )
    ap.add_argument(
        "--data-dir", type=str, default=DEFAULT_DATA_DIR, help=f"数据目录 (容器内路径, default: {DEFAULT_DATA_DIR})"
    )
    ap.add_argument(
        "--nsys-dir", type=str, default=DEFAULT_NSYS_DIR, help="nsys 输出目录 (容器内路径, default: {data_dir}/nsys)"
    )
    ap.add_argument("--model", type=str, default=DEFAULT_MODEL, help=f"模型路径 (default: {DEFAULT_MODEL})")
    ap.add_argument("--workdir", type=str, default=DEFAULT_WORKDIR, help=f"工作目录 (default: {DEFAULT_WORKDIR})")

    # 模型参数
    ap.add_argument("--tp-size", type=int, default=DEFAULT_TP_SIZE, help=f"TP size (default: {DEFAULT_TP_SIZE})")
    ap.add_argument("--ep-size", type=int, default=DEFAULT_EP_SIZE, help=f"EP size (default: {DEFAULT_EP_SIZE})")
    ...
    # 操作
    ap.add_argument("--cleanup", action="store_true", help="清理容器中残留的 sglang/nsys/torch 进程")
