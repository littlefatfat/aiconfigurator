...
def _load_nvtx_ranges(conn: sqlite3.Connection, forward_mode_filter: Optional[str] = None) -> list[dict]:
    ...
    """
    cur = conn.cursor()
    cur.execute("SELECT start, end, text FROM NVTX_EVENTS WHERE text LIKE '%Scheduler.run_batch%' ORDER BY start")
    ranges = []
...
def _load_kernels_in_range(
    ...
    return [{"start": r[0], "end": r[1], "name": r[2], "dur_ns": r[3]} for r in cur.fetchall()]
...
def extract_nsys_per_op(
    ...
    print(f"[nsys] {num_iters} iterations found, using iter #{selected_idx} (dur={selected_range['dur_ms']:.3f}ms)")
    ...
    if multi_rank and num_layers > 0:
        per_op_totals = _aggregate_multi_rank_moe(conn, selected_range, per_op_totals, num_layers, layer_details_d0)
...
def _aggregate_multi_rank_moe(
    ...
    for dev in all_devices:
        if dev == 0:
            continue
        kernels_dev = _load_kernels_in_range(conn, selected_range["start"], selected_range["end"], device_id=dev)
...
def print_comparison(
    ...
    print()
    print("=" * 100)
    print("  AIC per-op 预估 vs nsys 实测 kernel 时延对比")
    print(
...
def main():
    ...
    ap.add_argument("--iter-index", type=int, default=None, help="使用第 N 次迭代 (0-based)，默认自动选取中位数")
    ap.add_argument("--device-id", type=int, default=0, help="GPU device ID (默认 0)")
    ap.add_argument("--multi-rank", action="store_true", help="跨所有 rank 做 MoE 负载均衡对齐：每层选 MoE 最长的 rank")
