...
def _get_session():
    ...
    _model = get_model(model_path=MODEL_PATH, model_config=model_config, backend_name=BACKEND_NAME)
...
def estimate_batch_per_op(reqs: list[RequestInfo], is_decode: bool) -> tuple[dict[str, float], RuntimeConfig, float]:
    """
...
def run_from_case_id(data_dir: str, case_id: int, csv_path: Optional[str] = None):
    ...
    print(f"[load] batch_type={batch_type}, batch_size={len(reqs)}, measured_latency={measured_latency_ms:.3f}ms")
...
def run_from_manual(phase: str, batch_size: int, isl: int, osl: int, prefix: int = 0):
    ...
    if is_decode:
        ...
        reqs = [RequestInfo(input_length=isl - prefix, past_kv_length=prefix) for _ in range(batch_size)]
...
def main():
    ap = argparse.ArgumentParser(description="AIC per-op 时延预估工具 — 支持通过 case_id 或手动参数运行")
    ...
    p_case.add_argument(
        "--csv-path", type=str, default=None, help="CSV 文件路径（默认 data-dir/csv/batches_output.csv）"
    )
