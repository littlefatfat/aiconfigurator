...
def _custom_build_class_(func, name: str, *bases, **kwargs):
    for hook in CLASS_HOOKS:
        if (hook.REGEX and re.search(hook.HOOK_CLASS_NAME, name)) or name == hook.HOOK_CLASS_NAME:
            module_name = None
            if isinstance(func, FunctionType):
                module_name = getattr(func, "__globals__", {}).get("__name__", "")
            if (hook.REGEX and re.search(hook.HOOK_MODULE_NAME, module_name)) or module_name == hook.HOOK_MODULE_NAME:
                logger.debug(
                    f"Hooking Class: {hook.__name__} into {module_name}|{name}"
                    + ("(Regex is enabled, which might cause unexpected behavior.)" if hook.REGEX else "")
                )
...
class C_SglangSchedulerHook(BaseHook):
    ...
    def hook(cls, target_class):
            ...
            for req in batch.reqs:
                if req.finished():
                    C_SglangSchedulerHook.REQUEST_INFOS.append(
                        {"rid": req.rid, "input_ids": req.origin_input_ids, "output_ids": req.output_ids}
                    )
        ...
        def wrapped_profile(self, *args, **kwargs):
            ...
            with open(f"{SGL_HOOK_REQ_INFO_DIR}/{filename_prefix}.request.jsonl", "w") as f:
            ...
            ProfileReqOutput = importlib.import_module("sglang.srt.managers.io_struct").ProfileReqOutput
