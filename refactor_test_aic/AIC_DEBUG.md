...
└── aic_infer_cmp_nsys_profile/
    ...
    ├── aic_infer_component.py       # AIC SDK per-op 分解调试
...
# Prefill per-op 分解
python3 aic_infer_component.py prefill

# Decode per-op 分解
python3 aic_infer_component.py decode

# Decode 指定 past_kv_length
python3 aic_infer_component.py decode 2048
```

输出包括：
- **Per-op breakdown**: 每个算子 (GEMM/MoE/Attention/Comm) 的估算时延
- **Decode MoE interpolation debug**: MoE 插值详情（query_num_tokens、采集点、插值结果、scale_factor）
- **Full raw result**: AIC SDK 原始输出

> 需要根据实际 case 修改脚本中的 `RuntimeConfig` 参数 (batch_size, isl 等)。
